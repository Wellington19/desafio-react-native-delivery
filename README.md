...
Aplicação desenvolvida para praticar o que foi aprendido no React Native junto com TypeScript, para criar um pequeno app para pedidos de comida. Essa será uma aplicação que irá se conectar a uma Fake API, e exibir e filtrar os pratos de comida da API e permitir a criação de novos pedidos, seguindo os requisitos propostos logo abaixo pelo desafio 11 do bootcamp GoStack da Rocketseat.
